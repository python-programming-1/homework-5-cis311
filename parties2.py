...
def get_most_noisy_city_and_borough(data):
    ...
    for entry in data:
        ...
        if (not entry['borough'] in num_borough_calls.keys()):
            if (entry['borough'] != 'Borough' || entry['borough'] != 'Unspecified'):
                num_borough_calls.update({entry['borough']: 0})
        else:
            num_borough_calls[entry['borough']] = int(entry['num_calls']) + 1
...
def get_quietest_city_and_borough(data):
    ...
    city_min = min(num_city_calls.keys(),  
    key = lambda x: num_city_calls[x])
    borough_min = min(num_borough_calls.keys(), 
    key = lambda x: num_borough_calls[x])
